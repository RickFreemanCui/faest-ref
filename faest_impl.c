...
ATTR_PURE static inline uint8_t* rsd_signature_c(uint8_t* base_ptr, unsigned int index,
...
ATTR_PURE static inline uint8_t* rsd_signature_u_tilde(uint8_t* base_ptr,
                                                       const resolved_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline uint8_t* rsd_signature_a1_tilde(uint8_t* base_ptr,
                                                        const resolved_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline uint8_t* rsd_signature_a2_tilde(uint8_t* base_ptr,
                                                        const resolved_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline uint8_t* rsd_signature_decom_i(uint8_t* base_ptr,
                                                       const resolved_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline uint8_t* rsd_signature_chall_3(uint8_t* base_ptr,
                                                       const resolved_paramset_t* params) {
  const unsigned int lambda_bytes = params->lambda / 8;
  return base_ptr + params->sig_size - sizeof(uint32_t) - IV_SIZE - lambda_bytes;
}

ATTR_PURE static inline uint8_t* rsd_signature_iv_pre(uint8_t* base_ptr,
                                                      const resolved_paramset_t* params) {
  return base_ptr + params->sig_size - sizeof(uint32_t) - IV_SIZE;
}

ATTR_PURE static inline uint8_t* rsd_signature_ctr(uint8_t* base_ptr,
                                                   const resolved_paramset_t* params) {
  return base_ptr + params->sig_size - sizeof(uint32_t);
}
...
ATTR_PURE static inline const uint8_t* rsd_dsignature_c(const uint8_t* base_ptr, unsigned int index,
                                                        const resolved_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline const uint8_t* rsd_dsignature_u_tilde(const uint8_t* base_ptr,
                                                              const resolved_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline const uint8_t* rsd_dsignature_d(const uint8_t* base_ptr,
                                                        const resolved_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline const uint8_t* rsd_dsignature_a1_tilde(const uint8_t* base_ptr,
                                                               const resolved_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline const uint8_t* rsd_dsignature_a2_tilde(const uint8_t* base_ptr,
                                                               const resolved_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline const uint8_t* rsd_dsignature_decom_i(const uint8_t* base_ptr,
                                                              const resolved_paramset_t* params) {
  const unsigned int lambda_bytes  = params->lambda / 8;
  const unsigned int ell_bytes     = params->l / 8;
  const unsigned int ell_hat_bytes = ell_bytes + 3 * lambda_bytes + UNIVERSAL_HASH_B;
...
ATTR_PURE static inline const uint8_t* rsd_dsignature_chall_3(const uint8_t* base_ptr,
                                                              const resolved_paramset_t* params) {
  const unsigned int lambda_bytes = params->lambda / 8;
  return base_ptr + params->sig_size - sizeof(uint32_t) - IV_SIZE - lambda_bytes;
}

ATTR_PURE static inline const uint8_t* rsd_dsignature_iv_pre(const uint8_t* base_ptr,
                                                             const resolved_paramset_t* params) {
  return base_ptr + params->sig_size - sizeof(uint32_t) - IV_SIZE;
}

ATTR_PURE static inline const uint8_t* rsd_dsignature_ctr(const uint8_t* base_ptr,
                                                          const resolved_paramset_t* params) {
  return base_ptr + params->sig_size - sizeof(uint32_t);
}

// RSD.Prove Dispatcher

// RSD OWF dispatchers
static inline void rsd_prove(uint8_t* a0_tilde, uint8_t* a1_tilde, uint8_t* a2_tilde,
                             const uint8_t* w, const uint8_t* u, uint8_t** V, const uint8_t* owf_in,
                             const uint8_t* owf_out, const uint8_t* chall_2,
                             const resolved_paramset_t* params) {
  switch (params->lambda) {
  case 320:
    rsd_320_prover(a0_tilde, a1_tilde, a2_tilde, w, u, V, owf_in, owf_out, chall_2, params);
    break;
  default:
    rsd_320_prover(a0_tilde, a1_tilde, a2_tilde, w, u, V, owf_in, owf_out, chall_2, params);
  }
...
static inline void rsd_verify(uint8_t* a0_tilde, const uint8_t* d, uint8_t** Q,
                              const uint8_t* chall_2, const uint8_t* chall_3,
                              const uint8_t* a1_tilde, const uint8_t* a2_tilde,
                              const uint8_t* owf_in, const uint8_t* owf_out,
                              const resolved_paramset_t* params) {
  switch (params->lambda) {
  case 320:
    rsd_320_verifier(a0_tilde, d, Q, owf_in, owf_out, chall_2, chall_3, a1_tilde, a2_tilde, params);
    break;
  default:
    rsd_320_verifier(a0_tilde, d, Q, owf_in, owf_out, chall_2, chall_3, a1_tilde, a2_tilde, params);
  }
...
void resolved_sign(uint8_t* sig, const uint8_t* msg, size_t msg_len, const uint8_t* owf_key,
  ...
  // ::4-5
  uint8_t rootkey[MAX_LAMBDA_BYTES], iv[IV_SIZE];
  hash_r_iv(rootkey, rsd_signature_iv_pre(sig, params), iv, owf_key, mu, rho, rholen, lambda);
  ...
  resolved_vole_commit(rootkey, iv, ell_hat, params, &bavc, rsd_signature_c(sig, 0, params), u, V);
  ...
  // used elsewhere.
  H2_context_t chall_2_ctx;
  hash_challenge_2_init(&chall_2_ctx, chall_1, rsd_signature_u_tilde(sig, params), lambda);
  {
    uint8_t V_tilde[MAX_LAMBDA_BYTES + UNIVERSAL_HASH_B];
    for (unsigned int i = 0; i != lambda; ++i) {
      // Step 11
  ...
  // ::13 witness provided by caller
  // ::14
  xor_u8_array(witness, u, rsd_signature_d(sig, params), ell_bytes);
  ...
  // ::21-22
  H2_context_t chall_3_ctx;
  hash_challenge_3_init(&chall_3_ctx, chall_2, a0_tilde, rsd_signature_a1_tilde(sig, params),
                        rsd_signature_a2_tilde(sig, params), lambda);
  ...
  for (; true; ++ctr) {
    ...
    // ::26
    uint16_t decoded_chall_3[MAX_TAU];
    if (!rsd_decode_all_chall_3(decoded_chall_3, chall_3, params)) {
      continue;
    }

    // :27
    if (resolved_bavc_open(rsd_signature_decom_i(sig, params), &bavc, decoded_chall_3, params)) {
      break;
    }
  }
  hash_clear(&chall_3_ctx);
...
int resolved_verify(const uint8_t* msg, size_t msglen, const uint8_t* sig, const uint8_t* owf_input,
  ...
  // ::3
  uint8_t iv[IV_SIZE];
  hash_iv(iv, rsd_dsignature_iv_pre(sig, params), lambda);
  ...
  // Step 12, 14 and 15
  H2_context_t chall_2_ctx;
  hash_challenge_2_init(&chall_2_ctx, chall_1, rsd_dsignature_u_tilde(sig, params), lambda);
  {
    const uint8_t* chall_3 = rsd_dsignature_chall_3(sig, params);
    uint8_t Q_tilde[MAX_LAMBDA_BYTES + UNIVERSAL_HASH_B];
    for (unsigned int i = 0; i != lambda; ++i) {
      // Step 12
      vole_hash(Q_tilde, chall_1, q[i], ell, lambda);
      // Step 14
      if (ptr_get_bit(chall_3, i)) {
        xor_u8_array(Q_tilde, rsd_dsignature_u_tilde(sig, params), Q_tilde, utilde_bytes);
      }
