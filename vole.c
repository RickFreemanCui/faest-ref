...
#endif
    unsigned int
    resolved_convert_to_vole(const uint8_t* iv, const uint8_t* sd, bool sd0_bot, unsigned int i,
                    unsigned int outlen, uint8_t* u, uint8_t* v, const resolved_paramset_t* params) {
  const unsigned int lambda        = params->lambda;
  const unsigned int tau_1         = params->tau1;
  const unsigned int k             = params->k;
  const unsigned int num_instances = bavc_max_node_index(i, tau_1, k);
...
void resolved_vole_commit(const uint8_t* rootKey, const uint8_t* iv, unsigned int ellhat,
                 const resolved_paramset_t* params, bavc_t* bavc, uint8_t* c, uint8_t* u,
                 uint8_t** v) {
  const unsigned int lambda       = params->lambda;
  const unsigned int lambda_bytes = lambda / 8;
  const unsigned int ellhat_bytes = (ellhat + 7) / 8;
  ...
  resolved_bavc_commit(bavc, rootKey, iv, params);
  ...
  for (unsigned int i = 0; i < tau; ++i) {
    // Step 6
    v_idx +=
        resolved_convert_to_vole(iv, sd_i, false, i, ellhat_bytes, ui + i * ellhat_bytes, v[v_idx], params);
    sd_i += lambda_bytes * bavc_max_node_index(i, tau_1, k);
  }
...
bool resolved_vole_reconstruct(uint8_t* com, uint8_t** q, const uint8_t* iv, const uint8_t* chall_3,
                      const uint8_t* decom_i, const uint8_t* c, unsigned int ellhat,
  ...
  uint16_t i_delta[MAX_TAU];
  if (!rsd_decode_all_chall_3(i_delta, chall_3, params)) {
    return false;
  }
  ...
  if (!resolved_bavc_reconstruct(&bavc_rec, decom_i, i_delta, iv, params)) {
    free(bavc_rec.s);
    return false;
  }
  ...
  for (unsigned int i = 0; i < tau; i++) {
    ...
    // Step: 7..8
    const unsigned int ki = resolved_convert_to_vole(iv, sd, true, i, ellhat_bytes, NULL, qtmp, params);
