...
void resolved_vole_commit(const uint8_t* rootKey, const uint8_t* iv, unsigned int ellhat,
    const resolved_paramset_t* params, bavc_t* vecCom, uint8_t* c, uint8_t* u,
    uint8_t** v);
...
#if defined(FAEST_TESTS)
unsigned int resolved_convert_to_vole(const uint8_t* iv, const uint8_t* sd, bool sd0_bot, unsigned int i,
                unsigned int outLenBytes, uint8_t* u, uint8_t* v,
                const resolved_paramset_t* params);
#endif
